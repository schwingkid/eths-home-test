...
export async function onRequestPost(context) {
  ...
  // 'room' = in the clubroom (in_person); 'learn' = joining virtually as a learner (online).
  // Teaching is NOT recorded here — see /api/teaching (separate record, permissioned, sponsor-verified).
  const mode = String(body.mode || 'room').trim().toLowerCase();
  const joinMode = mode === 'learn' ? 'online' : 'in_person';
  ...
  await db.prepare(
    `INSERT INTO attendance (session_id, member_id, join_mode, source)
     VALUES (?, ?, ?, 'portal')
     ON CONFLICT(session_id, member_id) DO NOTHING`
  ).bind(session.id, member.id, joinMode).run();

  const attendance = await db.prepare(
    `SELECT checked_in_at, join_mode FROM attendance
  ...
  return json({
    ...
    joinMode: attendance?.join_mode || joinMode,
  });
}
