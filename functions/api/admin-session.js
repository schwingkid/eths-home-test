...
export async function onRequestGet(context) {
  ...
  let count = 0;
  if (session) {
    const row = await db.prepare(`SELECT COUNT(*) AS n FROM attendance WHERE session_id=?`).bind(session.id).first();
    count = Number(row?.n || 0);
  }
  return json({ ok:true, session: session || null, attendanceCount: count });
}
